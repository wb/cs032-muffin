...
namespace _3D_Renderer
{
    ...
    public class Renderer : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            m_models.Add(LoadModel("box", out boxTexture));
            m_model_textures.Add(boxTexture);

            m_parser.loadLevel(m_objects, m_models);
            player = new GameObject((Model)m_models.ElementAt((int)ModelName.BOX), ModelType.HUMAN, ModelName.BOX, new Vector3(100,24,100), Vector3.Zero, false, new Vector3(60, 60, 60));
            m_objects.Add(player);

            GameObject thing = new GameObject((Model)m_models.ElementAt((int)ModelName.BOX), ModelType.OBJECT, ModelName.BOX, new Vector3(100, 84, 100), Vector3.Zero, false, new Vector3(60, 60, 60));
            m_objects.Add(thing);

            Random random = new Random();
            for (int i = 0; i < 10; i++)
            {
                GameObject temp = new GameObject((Model)m_models.ElementAt((int)ModelName.BOX), ModelType.OBJECT, ModelName.BOX, new Vector3(random.Next(0, 600), random.Next(600, 1200), random.Next(0, 600)), Vector3.Zero, false, new Vector3(60, 60, 60));
                m_objects.Add(temp);
            }
            SetUpVertices();
            SetUpCamera();
        ...
        {
            camera = new GameCamera(2 * (new Vector3(-80, 50, 70)), new Vector3(0, 0, 0), device.Viewport.AspectRatio);
        }

        private void SetUpLights() {
            LightPos[0] = new Vector3(-20, 100, 20);
            LightPos[1] = new Vector3(20, 100, -20);
            LightPos[2] = new Vector3(-30, 100, 0);
        ...
        {
            _physicsEngine.update(m_objects, (float)gameTime.ElapsedGameTime.TotalSeconds);
            ...
            if(gstate.IsConnected){
                ...
                if(gstate.Buttons.A == ButtonState.Pressed && gstate.Buttons.B == ButtonState.Released){
                    player._position += ((ModelDir) *.4f);
                    player.applyForce(ModelDir * 100, Vector3.Zero);
                }
        ...
        private void DrawAll(String technique, int index)
        {
            ...
            foreach (GameObject o in m_objects) {
                Matrix worldMatrix = Matrix.CreateFromYawPitchRoll(o.Rotation.Y, o.Rotation.X, 0.0f) *
                                     Matrix.CreateTranslation(o._position) *
